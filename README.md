...
## Game Launch Stability

Some games can briefly starve Windows audio scheduling while they launch or compile shaders. `Manna Sound Sync` now starts with the stable receiver behavior enabled by default:

- larger receiver buffer
- higher receiver process priority when Windows allows it
- playback-device reset after repeated underruns or a long packet gap

There are no audio modes to choose during normal use. The app is meant to be on or off.

Source users can run the same stable receiver path manually:
